...
/*global variables */
double Gamma;
double *DataXR, *DataYR, *DataZR, *DOrX, *DOrY, *DOrZ, *DOr, *DORStar, *DOR, *DORStarX, *DORStarY, *DORStarZ;
double *MaX, *MaY, *MaZ;
...
int main(){
		  ...
		  for(int k=0;k<BNum;k++)
		  {
			  Theta = 2*((k+1)-1)*PI/BNum; // Theta = 2*(k-1)*PI/BNum;
		  		...
		  		for(int j=0;j<DSNum;j++)
					...
					/*(*DataXR) = DR*cos(OmegaR*(*Time)+atan2(DY,DX)+Theta);
					(*DataYR) = DR*sin(OmegaR*(*Time)+atan2(DY,DX)+Theta);
					(*DataZR) = DZ*(*Ones(TNum));*/
					//DataXR = linSpace(TNum);
					
					//here the 5thLoop has been consturcted inside of 5thLoop
					
					
					/*pthread_t thread1;
					const char *message1 = "Thread 1";
					int iter1;
					iter1 = pthread_create(&thread1, NULL,fifthLoop(3600, 1, OmegaR, DX, DY, DZ, DR, OX, OY ,OZ, Theta, DataXR, DataYR, DataZR, DOrX, DOrY, DOrZ, DOr),message1);
					if(iter1)
					{
						fprintf(stderr,"Error -pthread_create() return code: %d\n",iter1);
						exit(EXIT_FAILURE);
					}
					//fifthLoop(3600, 1, OmegaR, DX, DY, DZ, DR, OX, OY ,OZ, Theta, DataXR, DataYR, DataZR, DOrX, DOrY, DOrZ, DOr );
					pthread_join(thread1,NULL);
					exit(EXIT_SUCCESS);
					*/
					 //fifthLoop(int TNum, int DSNum, double OmegaR, double Gamma, double *MaX, double *May, double *MaZ, double DX, double DY, double DZ, double DR, double OX, double OY,\
					...
					fifthLoop(3600, 1, OmegaR, Gamma, MaX, MaY, MaZ, DX, DY, DZ, DR, OX, OY ,OZ, Theta, DataXR, DataYR, DataZR, DOrX, DOrY, DOrZ, DOr, DORStar, DOR, DORStarX,\
							 DORStarY, DORStarZ);
  ...
  /*for(int i=0;i<TNum;i++)
	  printf("Index[%d]: Time: %g		DataXR: %g		DataYR: %g		DataZR:  %g 	\n",i,Time[i],DataXR[i],DataYR[i],DataZR[i]);*
  void call_fifthLoop;
  call_fifthLoop = fifthLoop();*/
