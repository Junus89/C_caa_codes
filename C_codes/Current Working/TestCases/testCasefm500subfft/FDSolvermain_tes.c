...
int main()
{
  ...
  //int NFFT = nextPower2(FNum);
	NFFT = (int)pow(2.0, ceil(log((double)FNum)/log(2.0)));
  //int NFFT = FNum;
  ...
  OTime = make_vector(OTime,NFFT); /* calling forOTime function and initializing it */
  ...
  /*-------- Calculating Time history of the Pressure spectrum -------*/ 
	...
	//double **OpTM;
	//OpTM = make_dmatrix(NFFT,1);
	//int i;

	x = (double *) malloc(FNum * sizeof(double));
	for(int i=0; i<FNum; i++)
		{
			x[i] = i;
		}
		...
		for(i=0;i<NFFT;i++)
		{
			X[2*i+1] = pXOYM[i][1]; // data[[odd][even]] the real part is [1,1,1,1,0,0,0,0]
			X[2*i+2] = 0.0; // all even parts automatically asigned with 0.0
			//printf("X[%d] = %4.8f\n",i,X[2*i+1]);
			X[i]=X[2*i+1];
			...
			//four1(X, NFFT, -1);
			//OpTM[2*i+1][1] = X[2*i+1];
			//printf("OpTM[%d][1]= %4.8f\n",i,OpTM[2*i+1][1]);
			//printf("Xfft[%d] = %4.8f\n",i,X[2*i+1]);--
		//for(i=0;i<NFFT;i++)
		//{
			//printf("Xf[%d] = %4.8f\n",i,X[i]);}
		  ...
		  FILE *fwrite2;
		  fwrite2 = fopen("FDPressureTH.txt","w");
		
	  
			for(int j=0;j<FNum;j++)
			  {
				  fprintf(fwrite2,"%12.6f\t %12.6f\n",OTime[j],X[2*j+1]*NFFT);
			  }
...
/*  FILE *fwrite1;
  ...
  printf("------checking-----: FRStarP[0] = %g\n",FRStarP[0]);
	printf("------checking-----: FRStarP[TNum-1] = %g\n",FRStarP[TNum-1]);
  ...
  printf("%s %s %s %d %lf %d %d %lf %lf %lf\n",fdName, sdName, odName,\
    	 BNum,R,TNum,FNum,MaX,MaY,MaZ);
  printf("OmegaR = %4.4f [rad/s], fR = %4.4f [hz], TR = %4.4f [s], OmegaM = %4.4f [rad/s]\n\n",OmegaR, fR, TR, OmegaM);
