...
for(int i=0;i<FNum;i++)
  ...
  double complex ** OpMUpHalf, **OpMHalfConj, **OpMLowerHalf,**OpTM;
  OpMUpHalf = make_dmatrix(FNum/2+1,1);
  OpMHalfConj = make_dmatrix(FNum/2+1,1);
  OpMLowerHalf = make_dmatrix(FNum/2+1,1);
  OpTM = make_dmatrix(FNum,1);
  
  
  for (int j=20;j<21;j++)
  		for (int k=0;k<FNum/2;k++){
		  OpMUpHalf[k][j]=pXOYM[k][j];
  			printf("OpMuHalf[%d][%d] = %g\n",k,j,OpMUpHalf[k][j]);
		}
  
  for (int j=20;j<21;j++)
	  for (int k=1;k<FNum/2+1;k++)
		  OpMHalfConj[k][j]=conj(OpMUpHalf[k][j]);
  
  for (int j=20;j<21;j++)
  	   for (int k=1;k<FNum/2+1;k++)
	   	OpMLowerHalf[k][j]=OpMHalfConj[FNum/2-k+1][j];
  
  /*for(int j=20;j<21;j++)
	  for (int s=0;s<128;s++)
		  OpTM[s][j] = OpMUpHalf[s][j];*/
  ...
  free_dmatrix(OpMUpHalf,FNum/2+1);
  free_dmatrix(OpMHalfConj,FNum/2+1);
  free_dmatrix(OpMLowerHalf,FNum/2+1);
  free_dmatrix(OpTM,FNum);
