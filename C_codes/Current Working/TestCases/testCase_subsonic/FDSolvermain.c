...
int main(){
  ...
  for(int n =0; n< FNum;n++)
  {
	  ...
	  double complex a1 = 0.0 + 0.0*I;
	  double complex b1 = 0.0 + 0.0*I;
	  double complex c1 = 0.0 + 0.0*I;

	  double complex a2 = 0.0 + 0.0*I;
	  double complex b2 = 0.0 + 0.0*I;
	  double complex c2 = 0.0 + 0.0*I;
			  ...
			  Theta = 2*((k+1)-1)*PI/BNum; // Theta = 2*(k-1)*PI/BNum;
		  		...
		  		for(int j=0;j<DSNum;j++)
				    ...
				    //fifthLoop(3600, 1, OmegaR, Gamma, C_0, MaX, MaY, MaZ, DX, DY, DZ, DR, OX, OY ,OZ, Theta, DataXR, DataYR, DataZR, DOrX, DOrY, DOrZ, DOr, DORStar, DOR, DORStarX,\
				  		 DORStarY, DORStarZ, DORX, DORY, DORZ, RGamma, Vx, Vy, Vz, Q, Lx, Ly, Lz, FxM, FyM, FzM, FxP, FyP, FzP, FRStarM, FRStarP, FRM, FRP,SP1);
						 
					PBD1[n][m][k][j] = Inicomp;
					fifthLoop(TNum, DT,1, OmegaR, Omega, ka, Gamma, C_0, MaX, MaY, MaZ, DX, DY, DZ, DR, OX, OY ,OZ, Theta, DataXR, DataYR, DataZR, DOrX, DOrY, DOrZ, DOr, DORStar, DOR, DORStarX,\
			    		 DORStarY, DORStarZ, DORX, DORY, DORZ, RGamma, Vx, Vy, Vz, Q, Lx, Ly, Lz, FxM, FyM, FzM, FxP, FyP, FzP, FRStarM, FRStarP, FRM, FRP, PBD1,PB1,P1,pF);
  ...
  //fifthLoop(3600, 1, OmegaR, Gamma, C_0, MaX, MaY, MaZ, DX, DY, DZ, DR, OX, OY ,OZ, Theta, DataXR, DataYR, DataZR, DOrX, DOrY, DOrZ, DOr, DORStar, DOR, DORStarX,\
		 DORStarY, DORStarZ, DORX, DORY, DORZ, RGamma, Vx, Vy, Vz, Q, Lx, Ly, Lz, FxM, FyM, FzM, FxP, FyP, FzP, FRStarM, FRStarP, FRM, FRP,PBD1,PB1,P1,pF);
  //fifthLoop(DataXR, DataYR, DataZR, DOrX, DOrY, DOrZ, DOr, DORStar, DOR, DORStarX,DORStarY, DORStarZ, DORX, DORY, DORZ, RGamma);
  ...
  /*printf("------checking-----: Omega = %g\n",Omega);
  printf("------checking-----: Omega = %g\n",Omega);
  printf("------checking-----: ka = %g\n",ka);
  printf("------checking-----: ka = %g\n",ka); */
  ...
  printf("------checking-----: Q[0] = %g\n",Q[0]);
  printf("------checking-----: Q[TNum-1] = %g\n",Q[TNum-1]);
  /*printf("------checking-----: SP1T_sum = %g  %gi\n",creal(SP1T_sum),cimag(SP1T_sum));
  printf("------checking-----: SP1T = %f + %fi\n",creal(SP1T),cimag(SP1T));
  printf("------checking-----: SP1B = %g + %gi\n",creal(SP1B),cimag(SP1B));
  printf("------checking-----: SP1E = %g + %gi\n",creal(SP1E),cimag(SP1E));
  printf("------checking-----: SP1 = %g + %gi\n",creal(SP1),cimag(SP1));
  printf("------checking-----: z1 = %f + %fi\n",creal(z1),cimag(z1));
  printf("------checking-----: try = %g + %gi\n",creal(try),cimag(try));
  
  printf("------checking-----: SP2T_sum = %g  %gi\n",creal(SP2T_sum),cimag(SP2T_sum));
  printf("------checking-----: SP2T = %g + %gi\n",creal(SP2T),cimag(SP2T));
  printf("------checking-----: SP2B = %g + %gi\n",creal(SP2B),cimag(SP2B));
  printf("------checking-----: SP2E = %g + %gi\n",creal(SP2E),cimag(SP2E));
  printf("------checking-----: SP2 = %g + %gi\n",creal(SP2),cimag(SP2));
  
  printf("------checking-----: SP3T_sum = %g  %gi\n",creal(SP3T_sum),cimag(SP3T_sum));
  printf("------checking-----: SP3T = %g + %gi\n",creal(SP3T),cimag(SP3T));
  printf("------checking-----: SP3B = %g + %gi\n",creal(SP3B),cimag(SP3B));
  printf("------checking-----: SP3E = %g + %gi\n",creal(SP3E),cimag(SP3E));
  printf("------checking-----: SP3 = %g + %gi\n",creal(SP3),cimag(SP3));*/
  ...
  free_dmatrix(pF,1);
  free_dmatrix(Op,ObsrSthetaNum-1);
  free_dmatrix(pXOYM,HalfNum);
