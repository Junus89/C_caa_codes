...
void fifthLoop(double OmegaR,double Omega, double MaX, double MaY, double MaZ, double ka, double **pF)
{
  ...
  for(int i=0;i<TNum;i++)
    ...
    DataYR[i] = DR*sin(OmegaR*Time[i]+atan2(DY,DX)+Theta);
		...
		/*
		SP1 = SP1T = SP1B = SP1E = Inicomp;
		SP2 = SP2T = SP2B = SP2E = Inicomp;
		SP3 = SP3T = SP3B = SP3E = Inicomp; */
		...
		SP2T += DT*z1*ka*FRM[i]/DORStar[i]*cexp(-z1*Omega*RGamma[i]);
		SP2B = (DT/2)*(z1*ka*FRM[1]/DORStar[0]+FRStarM[1]/pow(DORStar[0],2))*cexp(-z1*Omega*RGamma[0]);
