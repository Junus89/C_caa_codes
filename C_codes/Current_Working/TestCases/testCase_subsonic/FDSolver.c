...
void fifthLoop(double OmegaR,double Omega, double ka, double *MaX, double *MaY, double *MaZ, double **pF)
{
  ...
  for(int i=0;i<TNum;i++)
    {
		...
		FxP[i] = -C_0*MaX[0]*Q[i]; // here MaX, MaY, and MaZ are zero, is given from the file so the remaining result is zero
		FyP[i] = -C_0*MaY[0]*Q[i];
		FzP[i] = -C_0*MaZ[0]*Q[i];
