...
int main(){
  ...
  for(int i=0; i<sizeof(GeomD);i++)
    {
      fscanf(fp_g,"%d %lg",&GeomD[i].BNum, &GeomD[i].R);
    }
  printf("----------GeometryData------------\n\n");
  printf("Bnum = %d, R = 4.4%g\n\n",GeomD[0].BNum, GeomD[0].R);
  ...
  printf("OmegaR = %4.4g [rad/s], fR = %4.4g [hz], TR = %4.4g [s], OmegaM = %4.4g [rad/s]\n\n",OmegaR, fR, TR, OmegaM);
  ...
  printf("-----checking------>Time[3409] = %4.4g\n",(double)Time[3409]);    /*checking Time */
  ...
  for(int i=0;i<sizeof(BladeMData);i++)
    {
      fscanf(fp_meshB,"%lg %lg %lg %lg %lg %lg",&BladeMData[i].DataX, &BladeMData[i].DataY, &BladeMData[i].DataZ, &BladeMData[i].nxData,&BladeMData[i].nyData,&BladeMData[i].nzData);

    }
  
  printf("----checking ---> DataX = %lg, DataY= %lg, DataZ = %lg\n, nxData = %lg\n, nyData = %lg\n, nzData = %lg\n",BladeMData[0].DataX, BladeMData[0].DataY, BladeMData[0].DataZ,BladeMData[0].nxData,BladeMData[0].nyData,BladeMData[0].nzData);
  int DSNum = 1;
  fclose(fp_meshB);
  ...
  //storing the data into the structure
  while(fscanf(fp_ObMesh,"%lg %lg %lg",&XO_temp, &YO_temp, &ZO_temp)!=EOF)
    {
  ...
  //check for Omega and K
  printf("Omega[1]=%4.4g   ka[1] = %4.4g\n",Omega[1],ka[1]);
  printf("Omega[256]=%4.4g   ka[256] = %4.4g\n",Omega[256],ka[256]);
  ...
  free(Omega);free(ka); 
  free(DX);free(DY);free(DZ);free(DR);
  free(DataXR);
  //free_4Ddmatrix(PBD1);free_4Ddmatrix(PBD2);free_4Ddmatrix(PBD3);
  //free_3Ddmatrix(PB1);free_3Ddmatrix(PB2);free_3Ddmatrix(PB3); 
  free_dmatrix(P1);free_dmatrix(P2);free_dmatrix(P3);
