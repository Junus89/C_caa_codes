...
int main(){
  ...
  fifthLoop(3600, 1, OmegaR, Gamma, C_0, MaX, MaY, MaZ, DX, DY, DZ, DR, OX, OY ,OZ, Theta, DataXR, DataYR, DataZR, DOrX, DOrY, DOrZ, DOr, DORStar, DOR, DORStarX,\
		 DORStarY, DORStarZ, DORX, DORY, DORZ, RGamma, Vx, Vy, Vz, Q, Lx, Ly, Lz, FxM, FyM, FzM, FxP, FyP, FzP, FRStarM, FRStarP, FRM, FRP);
  ...
  printf("------checking-----: RGamma[3333] = %g\n",RGamma[3333]);
  printf("------checking-----: Vx[65] = %g\n",Vx[65]);
  printf("------checking-----: Vy[2643] = %g\n",Vy[2643]);
  printf("------checking-----: Vz[3265] = %g\n",Vz[3265]);
  printf("------checking-----: Q[3265] = %g\n",Q[3265]);
  printf("------checking-----: OmegaM = %g\n",OmegaM);
  printf("------checking-----: Lx[3265] = %g\n",Lx[25]);
  printf("------checking-----: Ly[3265] = %g\n",Ly[63]);
  printf("------checking-----: Lz[3265] = %g\n",Lz[544]);
  printf("------checking-----: FxM[3265] = %g\n",FxM[255]);
  printf("------checking-----: FyM[3265] = %g\n",FyM[465]);
  printf("------checking-----: FzM[3265] = %g\n",FzM[1365]);
  printf("------checking-----: FxP[3265] = %g\n",FxP[5]); // here MaX, MaY, and MaZ are zero, is given from the file so the remaining result is zero
  printf("------checking-----: FyP[3265] = %g\n",FyP[365]);
  printf("------checking-----: FzP[3265] = %g\n",FzP[3565]);
  printf("------checking-----: FRStarM[3265] = %g\n",FRStarM[1485]);
  printf("------checking-----: FRStarP[3265] = %g\n",FRStarP[3595]);
  printf("------checking-----: FRM[1791] = %g\n",FRM[1791]);
  printf("------checking-----: FRP[3595] = %g\n",FRP[3595]);
